...
with open(RELATIVE_CONFIG_PATH + DAILY_REPORTS_COLLECTION + '.csv', 'r') as reports_fh:
    for reports_row in reports_fh:
        reports_row = reports_row.rstrip()
        if reports_row:
            (device_id, avg_value, min_value, max_value, date) = reports_row.split(',')
        reports_data = {'device_id': device_id, 'avg_value': avg_value, 'min_value': min_value, 'max_value': max_value, 'date': date}
